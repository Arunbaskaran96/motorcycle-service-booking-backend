    ...
    try {
        ...
        if(user){
            ...
            if(compare){
                ...
                const token=jwt.sign(userData,jwt_sct,{expiresIn:"50m"})
...
router.get("/usercard",AuthVerify,UserVerify,(req,res)=>{
    const email=req.Token.email
    const user=UserModel.findOne({email:email}).then(result=>{
        if(result){
